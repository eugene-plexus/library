...
`src/eugene_plexus_library/_generated/`.
...
GENERATED_DIR = REPO_ROOT / "src" / "eugene_plexus_library" / "_generated"
...
# library.yaml alone pulls in everything it needs from
# components/common.yaml. One input, one output module: simpler imports.
# openapi/library.yaml first appears in specs 8288926 (M2), so this path
# is only valid at a pin at or after that commit.
SPECS_TO_GENERATE = [
    ("openapi/library.yaml", "models.py"),
]
